...
- **Football data:** results update **automatically** via `LlmWebSearchProvider` (`FOOTBALL_PROVIDER=llm`, Anthropic Haiku + web_search tool) feeding the idempotent `/api/cron/update-results` route, triggered every 15 min by **Supabase pg_cron + pg_net** (Vault secrets `app_base_url`/`cron_secret`) — not by a Vercel cron. Four polls per match (1st half 20–45′, half-time 45–70′, 2nd half 80–110′, full-time from +115′ with retry-until-FT); `finished` only on explicit full-time confirmation; auto-rescore on finish. Provider failures are NEVER silent: `getLiveMatches` returns `LiveMatchesResult` (`matches`, `candidatesInWindow`, `providerError`) and the cron/sync JSON exposes `candidates`/`providerError` (visible in `net._http_response` — a dead Anthropic key looked like "0 matches" for 10 days, ADR-0023). Admin manual entry + recalc remain authoritative overrides. See `docs/decisions/0009-live-results-llm-web-search.md` (supersedes 0002) + 0023.
